...
public class HexTilemapCaveGenerator: MonoBehaviour {
    ...
    [SerializeField] TileBase slowTile = null;
    ...
    private HexCaveGenerator caveGenerator;

    void Start()  {
        //To get the same random numbers each time we run the script
        Random.InitState(100);

        caveGenerator = new HexCaveGenerator(randomFillPercent, gridSize);
    ...
    private void GenerateAndDisplayTexture(int[,] data) {
        for (int y = 0; y < gridSize; y++) {
            for (int x = 0; x < gridSize; x++) {
                ...
                var tile = data[x, y] == 1 ? wallTile: getRandomFloor();
                tilemap.SetTile(position, tile);
            }
        }
    }

    private TileBase getRandomFloor()
    {
        return Random.Range(1,6) == 3 ? slowTile : floorTile;
    }
}
